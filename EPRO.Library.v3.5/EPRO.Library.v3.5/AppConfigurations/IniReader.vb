...
#Region "Constructors"
        ...
        Public Sub New(ByVal iniFilePath As String,
            ...
            Catch e As Exception
                ...
                Modules.MyLogFile.Write(
                    New EException("Error While Reading File: " & iniFilePath, e)
                    )
                ...
                If (f.Trim() <> String.Empty) Then
                    ...
                    If (Lines.Length > 0) Then
                        ...
                        For Each Line As String In queries.Split(New String() {LineVDelimiter}, StringSplitOptions.RemoveEmptyEntries)
                            ...
                            Dim pTestKey = Line.Trim(New Char() {CChar(" "), Chr(9), Chr(13)})
                            ...
                            Dim keyValue As String() = Strings.Split(Line, KeyValuePairDelimiter, 2)
                            If (keyValue.Length = 2) Then

                                REM if error occured here, it means user entered same key more than once
                                If (Not Me.___isCaseSensitive) Then
                                    Me.IniDetails.Add(
                                       vAppendSectionToKeyName & keyValue(0).ToLower().Trim(New Char() {CChar(" "), Chr(9), Chr(13)}),
                                        keyValue(1).Trim()
                                        )
                                Else
                                    Me.IniDetails.Add(
                                         vAppendSectionToKeyName & keyValue(0).Trim(New Char() {CChar(" "), Chr(9), Chr(13)}),
                                         keyValue(1).Trim()
                                         )
